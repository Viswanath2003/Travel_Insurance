package com.insurance.document.repository;

import com.insurance.document.entity.GeneratedDocument;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.Pageable;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.stereotype.Repository;

import java.util.List;

@Repository
public interface GeneratedDocumentRepository extends JpaRepository<GeneratedDocument, String> {

    List<GeneratedDocument> findByEntityTypeAndEntityId(String entityType, String entityId);

    Page<GeneratedDocument> findByGeneratedBy(String generatedBy, Pageable pageable);
}
